...
export function orderChosen5(cards: readonly Card[]): Card[] {
  if (cards.length !== 5) {
...
export function formatChosen5Short(cards: readonly Card[]): string[] {
  return orderChosen5(cards).map(formatCardShort);
}

function rankToSymbol(rank: number): string {
  switch (rank) {
    case 14:
      return "A";
    case 13:
...
function compareSuitAsc(a: Card, b: Card): number {
...
type RankGroup = { rank: number; count: number; cards: Card[] };
...
function straightSequenceIfAny(cards: readonly Card[]): number[] | null {
